...
const lensDistortionShader = new THREE.ShaderMaterial({
  vertexShader: Shaders.basicVS,
  fragmentShader: Shaders.lensDistortionFS,
  uniforms: {
    tDiffuse: uniforms.tDiffuse,
    resolution: uniforms.resolution
  }
...
const convoMatrix = new THREE.ShaderMaterial({
  vertexShader: Shaders.basicVS,
  fragmentShader: Shaders.convoMatrixFS,
  uniforms: {
    tDiffuse: uniforms.tDiffuse,
    resolution: uniforms.resolution
  }
...
composer.addPass(new ShaderPass(lensDistortionShader))
composer.addPass(new ShaderPass(chromaShader))
composer.addPass(new ShaderPass(convoMatrix))
...
// lights
const ambientLight = new THREE.AmbientLight(0x99ff99, 0.3)
scene.add(ambientLight)

const topLight = new THREE.PointLight(0xeeffee, 1000000)
topLight.position.y = 1000
topLight.position.z = 100
scene.add(topLight)
...
//sky
const sky = new Sky()
const sun = new THREE.Vector3(0, 100000, 2000000)
sky.scale.setScalar(450000)
scene.add(sky)

const skyUnif = sky.material.uniforms
skyUnif['turbidity'].value = 15
skyUnif['rayleigh'].value = 7
skyUnif['mieCoefficient'].value = 0.08
skyUnif['mieDirectionalG'].value = 0.1
skyUnif['sunPosition'].value.copy(sun)
